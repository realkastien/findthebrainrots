...
function Zone.new(container)
	...
	for _, triggerType in triggerTypes do
		local activeConnections = 0
		local previousActiveConnections = 0
		for i, triggerEvent in triggerEvents do
...
function Zone:_calculateRegion(tableOfParts, dontRound)
	local bounds: { Max: any, Min: any } = {["Min"] = {}, ["Max"] = {}}
	for boundType, details in bounds do
		details.Values = {}
	...
	for boundName, boundDetail in bounds do
		for _, v in boundDetail.Values do
	...
	local boundMin = Vector3.new(table.unpack(minBound))
	local boundMax = Vector3.new(table.unpack(maxBound))
...
function Zone:_displayBounds()
	if not self.displayBoundParts then
		...
		for boundName, boundCFrame in boundParts do
			local part = Instance.new("Part")
			part.Anchored = true
...
function Zone:_update()
	...
	for _, zonePart in zoneParts do
		local _, shapeName = pcall(function() return zonePart.Shape.Name end)
	...
	zonePartsIgnorelist.FilterType = Enum.RaycastFilterType.Include
	...
	for _, part in zoneParts do
		for _, prop in partProperties do
	...
	for _, holder in holders do
		for _, event in containerEvents do
...
function Zone:_updateOccupants(trackerName, newOccupants)
	...
	local signalsToFire = {}
	for occupant, prevItem in previousOccupants do
	...
	for occupant, _ in newOccupants do
...
function Zone:_updateTouchedConnection(triggerType)
	...
	for _, basePart in self.zoneParts do
...
function Zone:_updateTouchedConnections()
	for triggerType, _ in self.touchedConnectionActions do
		local touchedJanitorName = "_touchedJanitor"..triggerType
		local touchedJanitor = self[touchedJanitorName]
		if touchedJanitor then
...
function Zone:findPart(part)
	...
	local touchingZoneParts = self.worldModel[methodName](self.worldModel, table.unpack(args))
...
function Zone:_getAll(trackerName)
	...
	if occupantsDict then
		for item, _ in occupantsDict do
...
function Zone:relocate()
	...
	if typeof(relocationContainer) == "table" then
		...
		for _, zonePart in self.zoneParts do
